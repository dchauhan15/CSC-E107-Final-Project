...
![**The Black-Scholes Equation**](BlackScholes.PNG)
...
AAPLCall <- read.csv("AAPL - CallData.csv")
head(AAPLCall)
...
pairs(~close+imp30+imp60+imp90+imp120,data=AAPLCallxts)    # scatterplot matrix
plot.zoo(AAPLCallxts)                                      # time series plots
par(mfrow=c(3,2))                                          # visual look for outliers
for(i in 1:5) qqnorm(AAPLCallxts[,i], main=colnames(AAPLCallxts)[i])
...
# Function  Price           #
#############################
plot_4_ranges_prices <- function(data, start_date, end_date, title) {
...
# Function  Log             #
#############################
plot_4_ranges_logs <- function(data, start_date, end_date, title) {
...
# Create Price plots
plot_4_ranges_prices(prices, begin_dates,  end_dates, "AAPL prices for:")
```
...
plot_4_ranges_logs(returns, begin_dates, end_dates, "AAPL log prices for:")
